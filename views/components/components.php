<section class="stack-l components" id="buttons">
	<h1>Buttons</h1>
	<hr>
	<h2>Buttons</h2>
	<div class="stack">
		<a class="btn btn-primary">Button primary</a>
		<a class="btn btn-secundary">Button secundary</a>
		<a class="btn btn-tertiary">Button tertiary</a>
	</div>
	<div class="stack">
		<a class="btn btn-o-primary">Button primary</a>
		<a class="btn btn-o-secundary">Button secundary</a>
		<a class="btn btn-o-tertiary">Button tertiary</a>
	</div>


	<h2>Buttons styles</h2>
	<div class="stack">
		<a class="btn btn-primary">Button default</a>
		<a class="btn btn-primary btn-round">Button round</a>
		<a class="btn btn-primary btn-circle"><i class="icon icon-heart"></i></a>
	</div>

	<h2>Large Buttons</h2>
	<div class="stack">
		<a class="btn btn-lg btn-primary">Button primary</a>
		<a class="btn btn-lg btn-secundary">Button secundary</a>
	</div>
	<div class="stack">
		<a class="btn btn-lg btn-o-primary">Button primary</a>
		<a class="btn btn-lg btn-o-secundary">Button secundary</a>
	</div>

	<h2>Super Large Buttons</h2>
	<div class="stack">
		<a class="btn btn-xl btn-primary">Button primary</a>
		<a class="btn btn-xl btn-secundary">Button secundary</a>
	</div>
	<div class="stack">
		<a class="btn btn-xl btn-o-primary">Button primary</a>
		<a class="btn btn-xl btn-o-secundary">Button secundary</a>
	</div>
	<h2>Link buttons</h2>
	<a class="btn btn-link">Button link</a>
	<a class="btn btn-link btn-secondary">Button link</a>

	<h2>Icon buttons</h2>
	<div class="stack">
		<a class="btn btn-action">
			<i class="icon icon-pen"></i>
		</a>
		<a class="btn btn-primary btn-action">
			<i class="icon icon-eye"></i>
		</a>
		<a class="btn btn-secundary btn-action">
			<i class="icon icon-pen"></i>
		</a>
		<a class="btn btn-warning btn-action">
			<i class="icon icon-eye"></i>
		</a>
		<a class="btn btn-success btn-action">
			<i class="icon icon-pen"></i>
		</a>
		<a class="btn btn-error btn-action">
			<i class="icon icon-eye"></i>
		</a>
		<a class="btn btn-information btn-action">
			<i class="icon icon-pen"></i>
		</a>
	</div>
	<div class="stack">
		<a class="btn btn-action btn-circle">
			<i class="icon icon-pen"></i>
		</a>
		<a class="btn btn-primary btn-action btn-circle">
			<i class="icon icon-eye"></i>
		</a>
		<a class="btn btn-secundary btn-action btn-circle">
			<i class="icon icon-pen"></i>
		</a>
		<a class="btn btn-warning btn-action btn-circle">
			<i class="icon icon-eye"></i>
		</a>
		<a class="btn btn-success btn-action btn-circle">
			<i class="icon icon-pen"></i>
		</a>
		<a class="btn btn-error btn-action btn-circle">
			<i class="icon icon-eye"></i>
		</a>
		<a class="btn btn-information btn-action btn-circle">
			<i class="icon icon-pen"></i>
		</a>
	</div>
	<h1 id="labels">Labels</h1>
	<hr>
	<div class="stack">
		<label class="label label--success">Collaborator label</label>
		<label class="label label--warning">Connection timeout label</label>
		<label class="label label--error">Error label</label>
		<label class="label label--information">Information label</label>
		<label class="label label--primary">Primary label</label>
	</div>
	<div class="stack">
		<label class="label label-o--success">Collaborator label</label>
		<label class="label label-o--warning">Connection timeout label</label>
		<label class="label label-o--error">Error label</label>
		<label class="label label-o--information">Information label</label>
		<label class="label label-o--primary">Primary label</label>
	</div>
	<div class="stack">
		<label class="label label--right label--primary">Right label</label>
		<label class="label label--left label--primary">Left label</label>
	</div>

	<div id="app">
		<h1 id="notifications">Notifications</h1>
		<hr>
		<div id="clone-16">
			<div class="alert stack-s" v-bind:class="['alert--'+selected]">
				<span class="alert__copy column-9">{{ selected[0].toUpperCase() + selected.slice(1) }}: phasellus sit amet mi nec odio posuere imperdiet ut a dui. Ut vitae risus ante. Proin varius ex in lorem maximus ullamcorper. Sed dictum ex sed sem semper, pellentesque posuere sem sagittis.</span>
				<span class="alert__btn">Ok, got it</span>
			</div>
		</div>
<!--		<div class="alert stack-s alert--warning">-->
<!--			<span class="alert__copy column-9">Warning: phasellus sit amet mi nec odio posuere imperdiet ut a dui. Ut vitae risus ante. Proin varius ex in lorem maximus ullamcorper. Sed dictum ex sed sem semper, pellentesque posuere sem sagittis.</span>-->
<!--			<span class="alert__btn">Ok, got it</span>-->
<!--		</div>-->
<!--		<div class="alert stack-s alert--error">-->
<!--			<span class="alert__copy column-9">Error: phasellus sit amet mi nec odio posuere imperdiet ut a dui. Ut vitae risus ante. Proin varius ex in lorem maximus ullamcorper. Sed dictum ex sed sem semper, pellentesque posuere sem sagittis.</span>-->
<!--			<span class="alert__btn">Ok, got it</span>-->
<!--		</div>-->
<!--		<div class="alert stack-s alert--information">-->
<!--			<span class="alert__copy column-9">Information: phasellus sit amet mi nec odio posuere imperdiet ut a dui. Ut vitae risus ante. Proin varius ex in lorem maximus ullamcorper. Sed dictum ex sed sem semper, pellentesque posuere sem sagittis.</span>-->
<!--			<span class="alert__btn">Ok, got it</span>-->
<!--		</div>-->
<!--		<div class="alert stack-s alert--primary">-->
<!--			<span class="alert__copy column-9">Primary: phasellus sit amet mi nec odio posuere imperdiet ut a dui. Ut vitae risus ante. Proin varius ex in lorem maximus ullamcorper. Sed dictum ex sed sem semper, pellentesque posuere sem sagittis.</span>-->
<!--			<span class="alert__btn">Ok, got it</span>-->
<!--		</div>-->
		<pre class="code-box" id="code-16">
			<span>{{notification}}</span>
			<v-select v-model="selected" :options="['success','warning', 'error', 'information', 'primary']"></v-select>
			<span>{{ notification2 }}</span>
		</pre>
	</div>

	<h1 id="badges">Badges</h1>
	<div class="stack" id="clone-15">
		<div class="label label--primary">
			<span class="badge badge-left">4</span>
			Label with badge
		</div>
		<div class="label label--primary">
			Label with badge
			<span class="badge badge-right">4</span>
		</div>
	</div>
	<pre class="code-box" id="code-15"></pre>

	<div class="stack" id="clone-14">
		<a class="btn btn-o-primary"><span class="badge badge-left">4</span>Button with badge</a>
		<a class="btn btn-o-secundary">Button with badge<span class="badge badge-right">4</span></a>
	</div>
	<pre class="row code-box" id="code-14"></pre>

</section>
<script src="https://unpkg.com/vue@2.3.2/dist/vue.js"></script>
<script src="https://unpkg.com/vue-select@latest"></script>
<script>
	Vue.component('v-select', VueSelect.VueSelect);

	var app = new Vue({
		el: '#app',
		data: {
			notification: '<div class="alert stack-s alert--',
			notification2: '">\n' +
			'\t<span class="alert__copy column-9">Success: phasellus sit amet mi nec odio posuere imperdiet ut a dui. Ut vitae risus ante. Proin varius ex in lorem maximus ullamcorper. Sed dictum ex sed sem semper, pellentesque posuere sem sagittis.</span>\n' +
			'\t<span class="alert__btn">Ok, got it</span>\n' +
			'</div>',
			selected: 'success'
		},
		methods: {}
	});
</script>
