<aside class="column sidebar">
  <div class="nav__item inset"> Overview </div>
  <div class="nav__item inset"> Colors </div>
  <div class="nav__item inset"> Typography </div>
  <div class="nav__item inset"> Buttons + Labels </div> <!-- Buttons and Labels -->
  <div class="nav__item inset"> Notifications </div> <!-- pusher.github.io/chameleon -->
  <div class="nav__item inset"> Iconography </div>
  <div class="nav__item inset"> Forms/inputs </div>
  <div class="nav__item inset"> Panels </div>
  <div class="nav__item inset"> Thumbnails </div>
</aside>
<article class="column-9">
  <?php include('views/components/colorscheme.php') ?>
  <?php include('views/components/typography.php') ?>
  <?php nclude('views/components/components.php') ?>
  <?php include('views/components/iconography.php') ?>
  <?php include('views/components/forms.php') ?>
  <?php include('views/components/panels.php') ?>
  <?php include('views/components/thumbnails.php') ?>
</article>